...
local function buildMainMenu()
    ...
    corner(TopBar, 12)
    squareOff(TopBar, Theme.Secondary, 12, true)
    ...
    -- подпись про INSERT - сдвинута левее, чтобы не липнуть к кнопке закрытия
    ...
    -- ==== БОКОВАЯ ПАНЕЛЬ (просто контейнер, без своего layout!) ====
    ...
    -- --- блок вкладок (свой отдельный layout, не трогает низ) ---
    ...
    -- --- профиль (юзер + аватар + UID), закреплён внизу ---
    ...
    UIDLabel.TextColor3 = Theme.SubText
    ...
    local tabNames = {"Main", "Main2", "Main3", "Main4"}
    local pages = {}
    local buttons = {}
